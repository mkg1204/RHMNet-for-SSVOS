...
class YtbScribble_MO_Train(data.Dataset):
    def __init__(self, root, max_object_num=3, clip_length=3, crop_size=(384,384), random_flip=False, minimum_area=100, 
                 additional_bg=True, init_skip=0, bidirection=False, dilate_kernel_size=19, ds_stride=4):
        self.root = root
        ...
        self.anno_dir = os.path.join(root, 'train_scribbles')

        self.all_videos = [i.split('/')[-1] for i in glob.glob(os.path.join(self.image_dir,'*'))]
        self.num_frames = {}
        self.videos = []
        self.img_files = {}
        self.anno_files = {}
        for _video in self.all_videos:
            tmp_imgs = glob.glob(os.path.join(self.image_dir, _video,'*.jpg'))
            tmp_annos = glob.glob(os.path.join(self.anno_dir, _video,'*.bmp'))
        ...
        print('Youtubu Seq number:', len(self.videos))

        self.K = 12
        self.skip = init_skip//5
